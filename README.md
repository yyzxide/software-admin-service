...
- 大文件上传：安装包分片上传、断点续传、分片原子覆盖、会话行锁、完成合并和上传会话消费。
- 上传治理：过期会话条件更新后再清理，避免与恢复上传竞态；失败时清理已落盘正式包和临时分片目录。
- 发布安全拦截：签名失败、未标记安全、处理失败或安全状态有风险的安装包不能审核通过或上架。
- 审核流程：提交审核、分配审核人、通过、驳回、审核历史，并校验审核人权限和任务归属。
- 操作审计：软件、版本、安装包、审核动作在事务提交后写入操作日志；写入失败记录错误但不反向破坏已提交业务。
...
- Token 采用 HS256 签名的标准 JWT，包含过期时间、会话版本和 `jti`。
...
当前测试通过情况以 `mvn clean test` 为准。最近一次本地验证为 **89 个测试、0 失败、1 个跳过**；跳过项是依赖 Docker 的 Testcontainers MySQL 集成测试，因为当前机器的 Docker API 与 Testcontainers 不兼容。
...
- 软件上传、编辑、发布、版本和安装包追加。
- 上下架幂等处理和条件状态迁移，避免并发审核覆盖业务状态。
- 上传文件大小、后缀和包格式校验。
- 分片上传会话、断点续传进度、分片合并、SHA256 校验和上传会话消费。
- 审核任务创建、审核人资格与归属、通过、驳回和版本级状态隔离。
- 操作日志写入、提交后失败隔离、日期范围边界、查询参数校验和展示内容提取。
...
- [后端面试知识地图](docs/interview/backend-study-guide.md)
