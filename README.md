...
- 安装包管理：按系统和 CPU 架构追加安装包变体，并支持本地模拟安全扫描。
- 大文件上传：安装包分片上传、断点续传、完成合并和上传会话消费。
- 上传治理：过期上传会话定时清理，自动删除临时分片目录。
- 发布安全拦截：签名失败、未扫描、扫描失败或安全扫描风险的安装包不能审核通过或上架。
- 审核流程：提交审核、分配审核人、通过、驳回、审核历史。
- 操作审计：软件、版本、安装包、审核动作写入操作日志。
...
- 保存后计算 SHA256，便于后续做完整性校验和安全扫描。
- 大文件可以先创建上传会话，再按分片上传；断网后通过已上传分片列表补传缺失分片。
- 分片合并后会再次校验文件大小和 SHA256，校验通过后才能绑定到软件、版本或安装包。
- 支持 `sha256` 摘要校验和 `sha256-rsa` 非对称签名验签。
- 后台会定时清理过期上传中的分片会话，避免 `.chunks` 临时目录长期堆积。
- 审核通过和上架前会检查安装包安全状态；签名失败、未扫描、扫描失败或扫描有风险时拒绝发布。
- 本地模拟扫描接口可将安装包标记为安全、风险或扫描失败，便于演示完整发布闭环。
- 软件、版本、安装包、审核动作会写入操作日志。
