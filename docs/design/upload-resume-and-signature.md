...
- 分片缺失：保持上传中，前端可以继续补传。
- 重复上传同一个分片：重新写入临时文件并原子替换，不能只凭文件大小相同判断成功；这样既支持重试，也能修复同大小但内容损坏的分片。
- 上传分片前通过 `SELECT ... FOR UPDATE` 锁定会话行，在同一事务里校验状态、写分片并更新进度，防止并发请求把进度覆盖回旧值。
- 完成合并前先将会话置为“合并中”，避免同一个上传会话被并发完成。
- 文件大小不一致、SHA256 不一致、签名失败：会话标记为失败。
- 过期清理先按“状态仍为上传/合并中且更新时间仍早于截止点”条件标记失败，仅成功标记后在事务提交后删除分片目录，避免与恢复上传竞态。
