...
> 我没有把权限简化成 admin/user 两种身份，而是做了 RBAC：管理员绑定角色，角色绑定权限点，接口通过注解声明需要的权限。拦截器在 Token 校验后读取权限注解，检查当前用户是否拥有对应权限。后台还提供账号管理和角色授权页面，可以创建运营、审核、只读等不同岗位账号，这样软件发布、审核通过、分类标签管理、日志查看都能做清晰的权限边界。
